...
class TemplateAnswerExtractor:
    ...
    def extract_answers(
        ...
        if question_type == 'list_request':
            ...
            answers = self._extract_explanatory_answers(question, qadi_results, max_answers)
        else:
    ...
    def _convert_to_direct_answer(
        ...
        if phase == 'questioning':
            ...
            direct_content = self._convert_question_to_action(idea.content, topic, answer_number)
        elif phase == 'abduction':
            # Convert hypotheses into creative solutions
            direct_content = self._convert_hypothesis_to_solution(idea.content, topic, answer_number)
        elif phase == 'deduction':
            # Convert logical analysis into concrete steps
            direct_content = self._convert_logic_to_steps(idea.content, topic, answer_number)
        elif phase == 'induction':
            # Convert patterns into strategic approaches
            direct_content = self._convert_pattern_to_strategy(idea.content, topic, answer_number)
        else:
    ...
    def _extract_topic_from_question(self, question: str) -> str:
        ...
        topic = re.sub(r'^(what|how|why|when|where|who)\s+(are|is|to|can|do|does)\s*', '', question.lower())
    ...
    def _convert_question_to_action(self, qadi_content: str, topic: str, number: int) -> str:
    ...
    def _convert_hypothesis_to_solution(self, qadi_content: str, topic: str, number: int) -> str:
    ...
    def _convert_logic_to_steps(self, qadi_content: str, topic: str, number: int) -> str:
    ...
    def _convert_pattern_to_strategy(self, qadi_content: str, topic: str, number: int) -> str:
...
class EnhancedAnswerExtractor:
    ...
    async def extract_answers(
        ...
        return self.template_extractor.extract_answers(question, qadi_results, max_answers)
