...
class TimeoutError(Exception):
    ...
    def __init__(
        self, message: str, phase: Optional[str] = None, elapsed: Optional[float] = None
    ):
        self.phase = phase
...
async def _log_progress(phase: str, timeout: float) -> None:
    ...
    try:
        while elapsed < timeout:
            ...
            logger.info(
                f"{phase} still running... ({elapsed:.0f}s elapsed, {remaining:.0f}s remaining)"
            )
    except asyncio.CancelledError:
        pass
...
def timeout_decorator(
    timeout: float, phase: Optional[str] = None, fallback: Optional[Any] = None
) -> Callable[..., Any]:
    """
