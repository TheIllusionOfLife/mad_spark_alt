...
class SmartAgentRegistry:
    ...
    async def setup_intelligent_agents(self) -> Dict[str, str]:
        ...
        if available_providers:
            ...
            if llm_setup_success:
                # Register LLM agents
                setup_status.update(
                    await self._register_llm_agents(available_providers)
                )
            else:
    ...
    async def _register_llm_agents(
    ...
    def _register_template_agents(self) -> Dict[str, str]:
        ...
        for agent_class, method, name in template_agents:
            try:
                ...
                logger.error(
                    f"Failed to register template agent for {method.value}: {e}"
                )
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "agent_preferences": {
                method.value: name for method, name in self._agent_preferences.items()
            },
            ...
            if agent:
                try:
                    ...
                    test_request = IdeaGenerationRequest(
                        problem_statement="Test connectivity", max_ideas_per_method=1
                    )
