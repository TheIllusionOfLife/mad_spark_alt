...
class SmartQADICycleResult(QADICycleResult):
    ...
    agent_types: Dict[str, str] = field(default_factory=dict)  # method -> agent_type (LLM/template)
...
class SmartQADIOrchestrator:
    ...
    async def run_qadi_cycle(self,
                           problem_statement: str,
                           context: Optional[str] = None,
                           cycle_config: Optional[Dict[str, Any]] = None) -> SmartQADICycleResult:
        """
        ...
        logger.info(f"Starting smart QADI cycle {cycle_id} for: {problem_statement[:100]}...")
        ...
        logger.info(f"Smart QADI cycle {cycle_id} completed in {result.execution_time:.2f}s")
        if total_llm_cost > 0:
            logger.info(f"Total LLM cost: ${total_llm_cost:.4f}")
            
        return result
    
    async def _run_smart_phase(self,
                             method: ThinkingMethod,
                             problem_statement: str,
                             context: Optional[str],
                             config: Dict[str, Any]) -> tuple[IdeaGenerationResult, str]:
        """
    ...
    async def _try_template_fallback(self,
                                   method: ThinkingMethod,
                                   request: IdeaGenerationRequest) -> Optional[IdeaGenerationResult]:
        """
    ...
    def _build_enhanced_context(self,
                               ...
                               *phase_results: Optional[IdeaGenerationResult]) -> str:
    ...
    def _synthesize_ideas(self,
                         phases: Dict[str, IdeaGenerationResult]) -> List[GeneratedIdea]:
    ...
    async def run_parallel_generation(self,
                                    problem_statement: str,
                                    thinking_methods: List[ThinkingMethod],
                                    context: Optional[str] = None,
                                    config: Optional[Dict[str, Any]] = None) -> Dict[ThinkingMethod, tuple[IdeaGenerationResult, str]]:
        """
        ...
        """
        logger.info(f"Running smart parallel generation with methods: {thinking_methods}")
        ...
        for method in thinking_methods:
            ...
            if agent:
                ...
                task = self._run_smart_phase(method, problem_statement, context, config or {})
                tasks.append(task)
            else:
                logger.warning(f"No agent available for {method.value}")
