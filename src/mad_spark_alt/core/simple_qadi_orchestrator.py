...
from .qadi_prompts import QADIPrompts, PHASE_HYPERPARAMETERS, calculate_hypothesis_score
...
class SimpleQADIOrchestrator:
    ...
    async def _run_questioning_phase(self, user_input: str, max_retries: int) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                self.total_llm_cost = getattr(self, 'total_llm_cost', 0.0) + response.cost
    ...
    async def _run_deduction_phase(
        self, user_input: str, core_question: str, 
        hypotheses: List[str], max_retries: int
    ) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # Extract the answer
                answer_match = re.search(r'ANSWER:\s*(.+?)(?=Action Plan:|$)', content, re.DOTALL)
    ...
    def _parse_hypothesis_scores(self, content: str, hypothesis_num: int) -> HypothesisScore:
