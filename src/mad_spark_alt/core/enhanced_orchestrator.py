...
class EnhancedQADIOrchestrator(SmartQADIOrchestrator):
    ...
    async def run_qadi_cycle_with_answers(
        ...
        if extract_answers and enhanced_result.synthesized_ideas:
            ...
            ideas_by_phase = self._group_ideas_by_phase(enhanced_result.synthesized_ideas)
            
            # Extract answers
            enhanced_result.extracted_answers = await self.answer_extractor.extract_answers(
    ...
    def _group_ideas_by_phase(self, ideas: List[GeneratedIdea]) -> Dict[str, List[GeneratedIdea]]:
    ...
    async def get_direct_answers(
        ...
        if result.extracted_answers:
            return [answer.content for answer in result.extracted_answers.direct_answers]
        else:
