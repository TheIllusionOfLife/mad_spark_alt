...
class RetryConfig:
    ...
    jitter: bool = True
    retry_on_errors: tuple = (ErrorType.RATE_LIMIT, ErrorType.NETWORK, ErrorType.TIMEOUT)
...
async def calculate_delay(attempt: int, config: RetryConfig, 
    ...
    **kwargs
    ...
    for attempt in range(1, retry_config.max_attempts + 1):
        try:
            ...
            if attempt == retry_config.max_attempts or not should_retry(error_type, retry_config):
                break
...
def handle_aiohttp_errors(response: aiohttp.ClientResponse, response_data: dict) -> None:
    ...
    if response.status == 401:
        ...
        error_message = response_data.get('error', {}).get('message', f'Server error: {response.status}')
        raise APIError(error_message, response.status)
    
    elif response.status >= 400:
        error_message = response_data.get('error', {}).get('message', f'Client error: {response.status}')
        raise APIError(error_message, response.status)
