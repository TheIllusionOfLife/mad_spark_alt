...
class LLMQuestioningAgent(ThinkingAgentInterface):
    ...
    async def generate_ideas(
        ...
        logger.info(
            f"{self.name} generating questions for: {request.problem_statement[:100]}..."
        )
        ...
        try:
            ...
            final_questions = await self._rank_and_select_questions(
                all_questions, max_questions, request.problem_statement, domain_analysis
            )
            ...
            logger.info(
                f"{self.name} generated {len(final_questions)} questions in {execution_time:.2f}s"
            )
...
Analyze this problem and provide the domain analysis in the specified JSON format."""
    ...
    def _select_questioning_strategies(
        self, domain_analysis: Dict[str, Any], config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
Using the {strategy_name} approach, generate insightful questions that will help explore and understand this problem more deeply. Focus on {strategy_focus} aspects while maintaining a {cognitive_approach} perspective."""
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse questions JSON for strategy {strategy_name}"
            )
            return []
        except Exception as e:
            logger.error(
                f"Question generation failed for strategy {strategy_name}: {e}"
            )
            return []
        ...
        try:
            ...
            for rank_idx in rankings[:max_questions]:
                if 0 <= rank_idx < len(questions):
                    questions[rank_idx].metadata["ranking_score"] = len(
                        rankings
            ...
            for question in questions:
                ...
                if len(selected) < max_questions:
                    if (
                        ...
                        or len(strategies_used) >= max_questions // 2
                    ):
