...
@pytest.mark.asyncio
async def test_evolution_produces_diverse_results():
    """Test that evolution with proper mutation produces diverse results."""
    ga = GeneticAlgorithm()
    
    # Create initial population
    initial_ideas = [
        GeneratedIdea(
            content=f"Initial idea {i} for testing evolution diversity",
            thinking_method=ThinkingMethod.QUESTIONING,
    ...
    # Configure with high mutation rate
    config = EvolutionConfig(
        ...
        max_parallel_evaluations=1,  # Must be at least 1
    )
    ...
    unique_contents = {idea.content for idea in result.best_ideas}
    
    # Should have some diversity (not all ideas identical)
    assert len(unique_contents) > 1
    
    # Check that ideas have generation metadata
    for idea in result.best_ideas:
        assert "generation" in idea.metadata