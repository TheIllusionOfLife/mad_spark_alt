...
class TestLLMProvider:
    ...
    def test_model_config_creation(self):
        ...
        assert config.model_name == TEST_GEMINI_MODEL
...
class TestUsageStats:
    ...
    def test_usage_stats_initialization(self):
        ...
        stats = UsageStats(LLMProvider.GOOGLE, TEST_GEMINI_MODEL)

        assert stats.provider == LLMProvider.GOOGLE
        assert stats.model == TEST_GEMINI_MODEL
...
class TestGoogleProvider:
    ...
    def test_get_available_models(self):
        """Test getting available Google models from registry."""
        provider = GoogleProvider("test_api_key")
        models = provider.get_available_models()

        # Should return all Gemini models from registry
        assert len(models) >= 2  # At least gemini-3-flash-preview and gemini-2.5-flash
        model_names = [m.model_name for m in models]
        assert TEST_GEMINI_MODEL in model_names  # Default model should be included
...
class TestIntegration:
    ...
    async def test_mock_llm_workflow(self):
        ...
        stats_key = f"google:{TEST_GEMINI_MODEL}"
