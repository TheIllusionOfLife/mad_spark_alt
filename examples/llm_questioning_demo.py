    ...
    for i, problem in enumerate(problems, 1):
        ...
        try:
            ...
            if "domain_analysis" in result.generation_metadata:
                ...
                console.print(f"\n🔬 Domain Analysis:")
                console.print(
                    f"   • Domain: {analysis.get('domain', 'N/A')} ({analysis.get('subdomain', 'N/A')})"
                )
                console.print(
                    f"   • Complexity: {analysis.get('complexity_level', 'N/A')}"
                )
                console.print(
                    f"   • Problem Type: {analysis.get('problem_type', 'N/A')}"
                )
                console.print(
                    f"   • Key Stakeholders: {', '.join(analysis.get('stakeholder_groups', []))}"
                )
            ...
            for j, idea in enumerate(result.generated_ideas, 1):
                ...
                console.print(
                    f"      💭 Reasoning: {idea.reasoning[:100]}..."
                    if idea.reasoning and len(idea.reasoning) > 100
                    else f"      💭 Reasoning: {idea.reasoning}"
                )
                if idea.metadata.get("stakeholder_relevance"):
                    console.print(
                        f"      👥 Stakeholders: {idea.metadata['stakeholder_relevance']}"
                    )
