...
async def showcase_llm_vs_template():
    ...
    if not has_llm:
        ...
        console.print(
            "This demo requires at least one of the following environment variables:"
        )
    ...
    for i, problem_data in enumerate(problems, 1):
        ...
        for phase_name, phase_result in result.phases.items():
            if phase_result.generated_ideas:
                best_idea = phase_result.generated_ideas[
                    0
                ]  # First idea (usually best ranked)
...
async def analyze_agent_intelligence():
    ...
    for phase_name, phase_result in result.phases.items():
        ...
        if phase_result.generated_ideas:
            total_length = sum(
                len(idea.content) for idea in phase_result.generated_ideas
            )
            metrics["avg_idea_length"] = total_length / len(
                phase_result.generated_ideas
            )
    ...
    for phase, metrics in intelligence_metrics.items():
        agent_display = (
            f"🤖 {metrics['agent_type']}"
            if "LLM" in metrics["agent_type"]
            else f"📝 {metrics['agent_type']}"
        )
...
async def cost_analysis():
    ...
    console.print(f"  • Ideas per Second: {total_ideas / max(total_time, 1e-9):.2f}")
