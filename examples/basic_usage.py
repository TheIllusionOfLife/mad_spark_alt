...
async def basic_evaluation_example():
    ...
    request = EvaluationRequest(outputs=[output], task_context="Creative writing task")
...
async def diversity_comparison_example():
    ...
    request = EvaluationRequest(
        outputs=outputs, task_context="Comparing AI responses about healthcare"
    )
    ...
    for i, result in enumerate(summary.layer_results.get(OutputType.TEXT, [])):
        if result.evaluator_name == "diversity_evaluator":
            ...
            print(
                f"  Semantic Uniqueness: {result.scores.get('semantic_uniqueness', 0):.3f}"
            )
            print(
                f"  Lexical Diversity: {result.scores.get('lexical_diversity', 0):.3f}"
            )
...
fibonacci_poetry(10)
    ...
    request = EvaluationRequest(
        outputs=[code_output], task_context="Creative coding task"
    )
