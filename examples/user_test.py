...
def display_welcome(show_full: bool = True):
    ...
    if available_providers:
        console.print(
            f"✅ LLM Providers Available: {', '.join(available_providers)}",
            style="green",
        )
        if show_full:
            console.print(
        ...
        console.print(
            "⚠️  No API keys found - using template-based generation", style="yellow"
        )
        if show_full:
            console.print(
...
def display_console_results(result):
    ...
    console.print(
        f"\n🎨 All Generated Ideas ({len(result.synthesized_ideas)} total):",
        style="bold",
    )
    ...
    while True:
        try:
            ...
            if problem.lower() == "help":
                ...
                console.print(
                    "• Type 'context:' followed by additional context (optional)"
                )
                console.print(
                    "• Type 'ideas:N' to set number of ideas per phase (default: 3)"
                )
                console.print(
                    "• Type 'save' to toggle file saving (currently: "
            ...
            if "ideas:" in problem.lower():
                ...
                if match:
                    ...
                    problem = re.sub(
                        r"ideas:\d+", "", problem, flags=re.IGNORECASE
                    ).strip()
...
Examples:
  ...
  %(prog)s --quiet --save "AI ethics challenges"
    ...
    parser.add_argument(
        "problem", nargs="*", help="Problem statement or question to analyze"
    )
    ...
    parser.add_argument(
        "--no-welcome", action="store_true", help="Skip welcome message and API status"
    )
